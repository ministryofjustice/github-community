...
def get_compliance_status(repository: AssetView | None) -> str:
    if not repository:
        return "unknown"
    ...
    if repository.data["default_branch_name"] != "main":
        return "fail"

    if repository.data["license"] != "mit":
        return "fail"
...
def get_repository_badge(repository_name: str):
    ...
    status = get_compliance_status(repository)
    ...
    if shields_response.status_code == 200:
        return Response(
            shields_response.content,
            content_type=shields_response.headers["Content-Type"],
        )
    else:
        return Response("Error fetching badge", status=500)
